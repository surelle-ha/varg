...
const { settings, setMaxChars, setDefaultVoice, setShowSystemMonitor, setShowMcpServer, setNotifyOnComplete } = useSettings()
...
async function sendTestNotification() {
  ...
  if (granted) sendNotification({ title: 'Varg', body: 'Notifications are working!' })
}
...
onMounted(async () => {
  try { bootEnabled.value = await autostartIsEnabled() } catch {}
})

async function toggleBoot() {
  try {
    if (bootEnabled.value) {
    ...
    console.error('[Varg] autostart toggle failed:', e)
  }
}
...
<template>
  <div class="mx-auto flex h-full w-full max-w-2xl flex-col gap-4 overflow-y-auto p-4">
    ...
    <div class="glass-panel rounded-xl p-4 space-y-4">
      ...
      <div class="flex items-center justify-between gap-4">
        <div class="min-w-0">
          <p class="text-[12px] text-secondary">Start on boot</p>
          <p class="mt-0.5 text-[11px] text-faded">Launch Varg automatically when Windows starts</p>
        </div>
        <button type="button"
          class="relative flex-shrink-0 h-5 w-9 rounded-full transition-colors duration-200"
          :class="bootEnabled ? 'bg-accent' : 'bg-subtle'"
          @click="toggleBoot">
          <span class="absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200"
            :class="bootEnabled ? 'translate-x-4' : 'translate-x-0'" />
        </button>
      </div>
      ...
      <div class="flex items-center justify-between gap-4">
        <div class="min-w-0">
          <p class="text-[12px] text-secondary">MCP Server</p>
          <p class="mt-0.5 text-[11px] text-faded">Show the MCP server panel in the sidebar</p>
        </div>
        <button type="button"
          class="relative flex-shrink-0 h-5 w-9 rounded-full transition-colors duration-200"
          :class="settings.showMcpServer ? 'bg-accent' : 'bg-subtle'"
          @click="setShowMcpServer(!settings.showMcpServer)">
          <span class="absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200"
            :class="settings.showMcpServer ? 'translate-x-4' : 'translate-x-0'" />
        </button>
      </div>
    </div>
    ...
    <!-- ── Notifications ─────────────────────────────────────────────────── -->
    <div class="glass-panel rounded-xl p-4 space-y-4">
      <h2 class="text-[12px] font-semibold text-secondary">Notifications</h2>

      <!-- Notify on complete -->
      <div class="flex items-center justify-between gap-4">
        <div class="min-w-0">
          <p class="text-[12px] text-secondary">Generation complete</p>
          <p class="mt-0.5 text-[11px] text-faded">Show a system notification when audio is ready</p>
        </div>
        <button type="button"
          class="relative flex-shrink-0 h-5 w-9 rounded-full transition-colors duration-200"
          :class="settings.notifyOnComplete ? 'bg-accent' : 'bg-subtle'"
          @click="setNotifyOnComplete(!settings.notifyOnComplete)">
          <span class="absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-transform duration-200"
            :class="settings.notifyOnComplete ? 'translate-x-4' : 'translate-x-0'" />
        </button>
      </div>
      ...
      <div class="flex items-center justify-between gap-4">
        <div class="min-w-0">
          <p class="text-[12px] text-secondary">Test notification</p>
          <p class="mt-0.5 text-[11px] text-faded">Send a test system notification now</p>
        </div>
        <button type="button"
          class="rounded-lg border border-white/10 px-3 py-1.5 text-[11px] font-medium text-secondary transition-ui hover:border-accent/40 hover:text-accent"
