...
const memHistory      = ref<number[]>(new Array(HIST).fill(0))
const activityHistory = ref<number[]>(new Array(HIST).fill(0))
...
function updateMemory() {
  ...
  if (m) {
    ...
    const pct = live.value.heapTotal > 0 ? Math.round((live.value.heapUsed / live.value.heapTotal) * 100) : 0
    memHistory.value = [...memHistory.value.slice(1), pct]
  }
}
...
function rafLoop(now: number) {
  if (lastRaf > 0) {
    ...
    live.value = { ...live.value, jsActivity: rounded }
    if (now - lastHistPush >= 1000) {
      lastHistPush = now
      activityHistory.value = [...activityHistory.value.slice(1), rounded]
    }
  }
  lastRaf = now
...
export function useSystemMonitor() {
  ...
  return { gpuInfo, cpuInfo, live, memHistory, activityHistory, probed, floatOpen }
}
