...
<template>
  ...
  <Transition name="drawer">
    <div v-if="generating && !visible" class="gen-bar flex-shrink-0">
      ...
      <div class="relative z-10 flex h-full items-center gap-3 px-5">
        ...
        <span class="text-[12px] font-medium tracking-wide text-secondary">Generating audio</span>
        <span class="text-[11px] tabular-nums text-faded/50">
          <template v-if="genChunk > 0 && genTotal > 1">Part {{ genChunk }}&thinsp;/&thinsp;{{ genTotal }}</template>
